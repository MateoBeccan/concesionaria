      ...
      <div class="card-body p-4">
        ...
        <div class="d-flex align-items-center gap-2 mb-4">
          <div
            ...
            style="width:28px;height:28px;font-size:.8rem;background:var(--color-primary)"
          >
            {{ paso }}
          </div>
          <h6 class="mb-0 fw-semibold" style="font-size:.9rem">{{ tituloPaso }}</h6>
        </div>

        <StepCliente  v-if="paso === 1" @seleccionado="onClienteSeleccionado" />
        <StepVehiculo v-if="paso === 2" @seleccionado="onVehiculoSeleccionado" @atras="paso = 1" />
        <StepConfirmar
          v-if="paso === 3"
          :cliente="cliente!"
          :vehiculo="vehiculo!"
...
import StepIndicator from '@/shared/composables/StepIndicator.vue';
...
function onClienteSeleccionado(c: ICliente) { cliente.value = c; paso.value = 2; }
function onVehiculoSeleccionado(v: IVehiculo) { vehiculo.value = v; paso.value = 3; }
